...
def integrated_lod(y, C):
    ...
    return (
        C
        + 31.4115 * t * t
        + 284.8435805251424 * np.cos(0.4487989505128276 * (t + 0.75))
    )
...
def DeltaT_error_estimate(y):
    ...
    if isinstance(y, (int, float)):
        ...
        return np.where(
            ...
            np.where(
                y < ytab[nytab - 1],
                eps_tab[np.searchsorted(ytab, y, "right") - 1],
                k2 * (y - 1875) ** 2,
            ),
        )
...
def DeltaT_with_error_estimate(y):
    ...
    if isinstance(y, (int, float)):
        ...
        for i, e in enumerate(eps):
            if e > 10:
                out[i] = str(round(dT[i])) + " \u00B1 " + str(round(e)) + " seconds"
            elif e < 0.09:
                out[i] = str(round(dT[i], 2)) + " \u00B1 " + str(e) + " seconds"
            elif e < 0.9:
                out[i] = str(round(dT[i], 1)) + " \u00B1 " + str(e) + " seconds"
            else:
                out[i] = str(round(dT[i])) + " \u00B1 " + str(e) + " seconds"
        return out
